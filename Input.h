...
class Input
{
  ...
  private:
    ...
    // uint8_t sensorId;  // FUTURE: use debuncer pin 
    // MyMessage msg;   // 
  ...
  public:
    ...
    void present()
    {
        ...
        // Serial_mysensors_logln("Input presented as sensorId: ", pin() ); // Id of sensor that this message concerns. 
    }
    ...
    bool sendState(bool & logLevel) {  // TODO: Rename state to logLevel and pin_state to pinHigh
        ...
        msg.setSensor(pin());
        msg.set(logLevel);
        // Serial_mysensors_logln("Sending state change of pin: ", pin(), " to val: ", logLevel ); 
        return send(msg);
    }
